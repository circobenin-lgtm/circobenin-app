...
const INTERVENANTS = {
  "Jean-Luc": {
    ...
    heures_prepa: 2,
    total_semaine: 12,
    cours_ids: [101, 102, 103, 104, 105, 106, 107, 108, 109, 110],
    extra: []
  },
  "Spéro": {
    code: "SPER531",
...
function InscriptionForm({ onPayer, onContact, preselect, onClearPreselect }) {
  ...
  const TARIFS_COTIS_AN = { 0: 0, 1: 150000, 2: 285000, 3: 420000 };
  const TARIFS_COTIS_TRIM = { 0: 0, 1: 50000, 2: 95000, 3: 140000 };
  const nbCren = Math.min(Math.max(form.creneaux ? form.creneaux.length : 1, 1), 3);
  const COTIS_TRIMESTRE = TARIFS_COTIS_TRIM[nbCren] || 140000;
  const COTIS_ANNEE = TARIFS_COTIS_AN[nbCren] || 420000;
  ...
  const handleSend = async (mode) => {
    ...
    const bodyAccueil = "<h2>Nouvelle inscription - " + type.titre + "</h2><p><b>Nom :</b> " + form.prenom + " " + form.nom + "</p><p><b>Naissance :</b> " + form.dateNaissance + "</p><p><b>Email :</b> " + form.email + "</p><p><b>Tel :</b> " + form.telephone + "</p><p><b>Discipline :</b> " + (Array.isArray(form.disciplines) && form.disciplines.length > 0 ? form.disciplines.join(", ") : "Non renseigné") + "</p><p><b>Créneau(x) :</b> " + (Array.isArray(form.creneaux) && form.creneaux.length > 0 ? form.creneaux.join(" | ") : "Non renseigné") + "</p><p><b>Formule :</b> " + formuleLabel + " — " + montant.toLocaleString() + " FCFA</p><p><b>Mode de paiement :</b> " + (mode === "enligne" ? "En ligne (FedaPay)" : "Sur place") + "</p>";
...
export default function App() {
  ...
  const [mgSujet, setMgSujet] = useState("Réinscription Circo Bénin 2026-2027");
  const [mgCorps, setMgCorps] = useState("Bonjour [prénom parent],\n\nNous espérons que vous allez bien !\n\nLa nouvelle rentrée Circo Bénin approche - 28 septembre 2026 dans notre nouveau local à Fidjrossè, à côté du marché Adjaha.\n\nComptez-vous réinscrire [prénom enfant] cette année ? Les places sont limitées, inscriptions dès le 14 septembre sur app.circobenin.com\n\nNous serions ravis de retrouver [prénom enfant] !\n\nL'''equipe Circo Bénin - +229 01 96 14 63 60");
  const [mgEnvoi, setMgEnvoi] = useState({});
  const [mgEnCours, setMgEnCours] = useState(false);
          ...
          {page === "message_groupe" && (
            <div>
              <div style={{ background: "linear-gradient(135deg, #2d7a4f, #1a5c38)", borderRadius: 20, padding: "28px 32px", color: "#fff", marginBottom: 24 }}>
                ...
                <div style={{ opacity: 0.85, fontSize: 14 }}>Personnalisez et envoyez un message à tous les parents en un clic</div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                <Card>
                  ...
                  <div style={{ marginBottom: 14 }}>
                    <label style={{ fontSize: 12, fontWeight: 700, color: C.gris, display: "block", marginBottom: 6 }}>
                      Message <span style={{ fontWeight: 400, color: C.gris }}>(utilisez [prénom parent] et [prénom enfant])</span>
                    </label>
                  ...
                  <Btn onClick={async () => {
                    const parents = elevesState.filter(e => e.emailParent || e.email).map(e => ({ nom: e.nom, prenom: e.prenom, prenomParent: e.nomParent || "", email: e.emailParent || e.email }));
                    setMgEnCours(true);
                    const res = {};
                    for (const p of parents) {
                      res[p.email] = "envoi..."; setMgEnvoi({...res});
                      ...
                      try { const r = await fetch("/api/send-email", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ from: "Circo Benin <accueil@circobenin.com>", to: p.email, subject: mgSujet, html }) }); res[p.email] = r.ok ? "✓ Envoyé" : "✗ Erreur"; }
                      catch { res[p.email] = "✗ Erreur"; }
                      setMgEnvoi({...res});
                      await new Promise(r => setTimeout(r, 600));
                    }
                    setMgEnCours(false);
                  }} disabled={mgEnCours}>
                    {mgEnCours ? "Envoi en cours..." : "Envoyer à " + elevesState.filter(e => e.emailParent || e.email).length + " parent(s)"}
                  </Btn>
                </Card>
                <Card>
                  <SectionTitle>Destinataires ({elevesState.filter(e => e.emailParent || e.email).length})</SectionTitle>
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 480, overflow: "auto" }}>
                    {elevesState.filter(e => e.emailParent || e.email).map((e, i) => (
                      <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", borderRadius: 8, background: C.grisClair, fontSize: 13 }}>
                        <div>
                          <div style={{ fontWeight: 600 }}>{e.prenom} {e.nomFamille}</div>
                          <div style={{ fontSize: 11, color: C.gris }}>{e.emailParent || e.email}</div>
                        </div>
                        <div style={{ fontSize: 12, fontWeight: 700, color: mgEnvoi[e.emailParent || e.email] === "✓ Envoyé" ? C.vert : mgEnvoi[e.emailParent || e.email] === "✗ Erreur" ? C.rouge : C.gris }}>
                          {mgEnvoi[e.emailParent || e.email] || "En attente"}
                        </div>
                      </div>
                    ))}
                    {elevesState.filter(e => e.emailParent || e.email).length === 0 && <p style={{ color: C.gris, fontSize: 13 }}>Aucun parent avec email enregistré.</p>}
                  </div>
                </Card>
              </div>
            </div>
